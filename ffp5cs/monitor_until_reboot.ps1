...
} -ArgumentList $Port, $BaudRate, $logFile
...
Write-Host ""
Write-Host "=== ANALYSE COMPLÈTE DES LOGS ===" -ForegroundColor Green
Write-Host ""

# Initialiser le rapport d'analyse
$report = @"
========================================
RAPPORT D'ANALYSE - MONITORING JUSQU'AU REBOOT
========================================
...
if (-not $logLines -or $logLines.Count -eq 0) {
    ...
    $report += "❌ ERREUR: Aucun log capturé`n"
...
if ($guruCount -gt 0) {
    ...
    foreach ($match in $guruMatches | Select-Object -First 10) {
        ...
        $context = if ($lineNum -gt 5) { $logLines[($lineNum-6)..($lineNum+5)] } else { $logLines[0..($lineNum+5)] }
        $report += "   Ligne $lineNum : $($match.Line.Trim())`n"
        $report += "   Contexte:`n"
...
if ($brownoutCount -gt 0) {
    ...
    foreach ($match in $brownoutMatches | Select-Object -First 5) {
        $report += "   Ligne $($match.LineNumber): $($match.Line.Trim())`n"
    }
} else {
...
if ($stackCount -gt 0) {
    ...
    foreach ($match in $stackMatches | Select-Object -First 5) {
        $lineNum = $match.LineNumber
        $context = if ($lineNum -gt 3) { $logLines[($lineNum-4)..($lineNum+3)] } else { $logLines[0..($lineNum+3)] }
        $report += "   Ligne $lineNum: $($match.Line.Trim())`n"
        $report += "   Contexte:`n"
        foreach ($ctxLine in $context) {
            $report += "     $ctxLine`n"
