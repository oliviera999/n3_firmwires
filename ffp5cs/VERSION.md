...
- **OTA** : publication `ffp5-wroom-prod` → `serveur/ota/esp32-wroom/` (canal prod).
- **Contenu** : identique à 14.13 (correctif OTA boot heap/retries/ets_printf) ; incrément pour cycle OTA après flash USB COM6.
- **Build** : `pio run -e wroom-prod`.
...
- **OTA** : publication `ffp5-wroom-beta` → `serveur/ota/esp32-wroom-beta/` (canal test) — validation OTA distante v14.09 → v14.10.
- **Build** : `pio run -e wroom-beta`.

---

## Version 14.09 - 2026-06-16

### Déploiement OTA canal beta (wroom-beta)

- **OTA** : publication `ffp5-wroom-beta` → `serveur/ota/esp32-wroom-beta/` (canal test) — validation OTA distante v14.08 → v14.09.
- **Build** : `pio run -e wroom-beta`.
...
- **Découpe des god-files restants** (méthodes membres réparties par TU / code mort, comportement identique, chaque étape validée par **vrai build `wroom-test`**) :
  ...
  |---|---|---|---|
  | `mailer.cpp` | 1653 | 870 | code mort `buildSystemInfoFooter` supprimé ; `mailer_queue.cpp` (file mail + envois async) |
  | `sensors.cpp` | 1690 | 544 | `sensor_ultrasonic.cpp`, `sensor_air.cpp` (3 classes → 1/fichier) |
  | `automatism.cpp` | 1519 | 658 | `automatism/automatism_refill.cpp` (`handleRefill*`), `automatism/automatism_display.cpp` (alertes + écrans) |
  | `nvs_manager.cpp` | 1179 | 714 | `nvs_manager_typed.cpp` (accesseurs typés) ; `NVSLockGuard` → header |
  | `display_view.cpp` | 1279 | 587 | `display_view_render.cpp` (format + `render*`), `display_view_show.cpp` (écrans `show*`) |
  | `web_client.cpp` | 1219 | 936 | `web_client_queue.cpp` (file NVS retry POST), `web_client_send.cpp` (`sendMeasurements`/`tryFetch`/`tryPush`) |
  | `ota_manager_download.cpp` | 1362 | 911 | `ota_manager_download_alt.cpp` (firmware micro-chunks + image LittleFS) |

  Décompositions **incrémentales** (un groupe cohésif par fichier, chaque étape validée par le build). **Aboutissement : plus aucun `.cpp` de `ffp5cs` au-dessus de 1000 lignes** (le plus gros restant, `ota_manager_download.cpp` 1362, scindé → 911). Bugs réels exposés+corrigés par le vrai build : garde `DISABLE_ASYNC_WEBSERVER` (web_routes), type incomplet `NVSLockGuard`, helper `cmToMm`, double-définition `oled_logo` (récurrente — `oled_logo.h` définit un tableau global **non**-`const` à liaison externe ; correctif : retirer l'`#include "oled_logo.h"` superflu du TU extrait qui ne l'utilise pas), externs `config`/`g_*`.
- **Découpe du god-header `config.h` (1128 → 38 l. d'« umbrella »)** : les ~25 namespaces de configuration répartis physiquement en sous-headers cohésifs — `config_secrets.h` (Secrets + validation PROFILE_PROD), `config_system.h`, `config_network.h`, `config_buffers.h`, `config_sensors.h`, `config_display.h`, `config_tasks.h`, `config_logging.h`. Les façades v13.60 (qui se contentaient de `#include "config.h"`) sont désormais **peuplées** (sens inversé) ; `config.h` les ré-inclut **dans l'ordre d'origine** car l'ordre porte des dépendances (`Secrets`→réseau, `SystemConfig::EPOCH_*`→`SleepConfig`, `BufferConfig`→`NVSConfig`) et la macro `#define Serial` doit rester **en dernier**. Interface `#include "config.h"` **inchangée** → zéro modif dans les ~37 `.cpp` consommateurs. Build `wroom-test` OK (RAM 37,9 % / Flash 51,1 %, identiques → neutre).
...
- Décomposition `web_server.cpp` (2084 → **898 lignes, −57 %**) : trois groupes d'endpoints extraits via `WebRoutes::register*Routes` (pattern status/ui) — **WiFi** (`/wifi/*` → `web_routes_wifi.cpp`), **NVS** (`/nvs*` + helpers `kNvsTypeTable`/`nvsTypeToStr`/`printNvsEntryToJson` → `web_routes_nvs.cpp`), **système** (`/update`, `/api/ota`, `/mailtest`, `/fs/format`, `/testota` → `web_routes_system.cpp`). `getWebParam()` rendu public. Comportement identique, build `wroom-test` OK.
- Décomposition `app_tasks.cpp` (1788 → 1458 lignes) :
  - le **pool statique de requêtes réseau** (`NetRequest`, slots dédiés OTA/Fetch/POST + alloc/free + accesseurs) extrait en module cohésif `net_request_pool.h/.cpp`.
  - **refonte amorcée** : `app_tasks_internal.h` (interface interne partagée — `g_ctx`/`g_sensorQueue` en `extern`, définitions inchangées) permet de répartir les corps de tâches FreeRTOS. `webTask` → `app_tasks_web.cpp`, `sensorTask` → `app_tasks_sensor.cpp` (déplacement verbatim).
  - **concern mail isolé** : réserve heap SMTP 32KB + `processMailQueueIfReady` → module `task_mail.h/.cpp` (API `allocMailReserveIfNeeded`/`processMailQueueIfReady`/`mailReserveReleaseIfInternal`).
...
- Décomposition `ota_manager.cpp` (2074 → 429 lignes) : les définitions des méthodes membres de `OTAManager` réparties sur plusieurs TU — **validation** (métadonnées/tailles/espace/sélection) → `ota_manager_validate.cpp` (343 l.) ; **téléchargement/flash** (metadata, firmware modern/fallback, updateTask) → `ota_manager_download.cpp` (1362 → **911 l.**), dont les chemins secondaires (firmware en micro-chunks `downloadFirmwareUltraRevolutionary` + image LittleFS `downloadFilesystem`) re-scindés vers `ota_manager_download_alt.cpp` (483 l.). Classe et comportement inchangés (déplacement verbatim). Build `wroom-test` OK.
