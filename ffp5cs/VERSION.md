...
- Décomposition `app_tasks.cpp` (1788 → 1614 lignes) : le **pool statique de requêtes réseau** (`NetRequest`, slots dédiés OTA/Fetch/POST + alloc/free + accesseurs) extrait en module cohésif et isolé `net_request_pool.h/.cpp` (déplacement verbatim, comportement identique). Le reste d'`app_tasks` (8 tâches FreeRTOS interdépendantes via queues) demande une refonte d'interface dédiée (différé). Build `wroom-test` OK.
