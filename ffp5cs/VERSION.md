...
- Décomposition `web_server.cpp` (2084 → **1040 lignes, −50 %**) : groupes d'endpoints **WiFi** (`/wifi/*`, ~483 l. → `web_routes_wifi.cpp`) et **NVS** (`/nvs*`, ~558 l. → `web_routes_nvs.cpp`) extraits via `WebRoutes::registerWifiRoutes` / `registerNvsRoutes` (pattern status/ui). Helpers NVS (`kNvsTypeTable`, `nvsTypeToStr/StrToType`, `printNvsEntryToJson`) déplacés avec leurs routes ; `getWebParam()` rendu public. Comportement identique, build `wroom-test` OK.
