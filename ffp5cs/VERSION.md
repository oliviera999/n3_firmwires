...
- Décomposition `app_tasks.cpp` (1788 → 1458 lignes) :
  ...
  - **concern mail isolé** : réserve heap SMTP 32KB + `processMailQueueIfReady` → module `task_mail.h/.cpp` (API `allocMailReserveIfNeeded`/`processMailQueueIfReady`/`mailReserveReleaseIfInternal`). Débloque l'extraction des tâches qui partagent la réserve (OTA, postSender).
  - Les tâches restantes (net/automation/ota/postSender, plus couplées) suivront le même pattern.
