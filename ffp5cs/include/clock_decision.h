#pragma once
// =============================================================================
// FFP5CS — Décisions d'horloge (pures) : plausibilité NTP + dérive RTC
// =============================================================================
// Extrait de power.cpp (chantier C4). Deux logiques numériques critiques :
//
...
//     sync. epochAbsDiff est réutilisé depuis epoch_util.h (déjà testé).
//
...
//
// Pur : <cmath>/<ctime> + epoch_util.h uniquement. Aucune dépendance Arduino.h
// ni config.h ; bornes/seuils passés en paramètres (mêmes valeurs par défaut que
// les originaux SleepConfig/SystemConfig) pour rester testable de façon autonome.
// L'I/O (RTC read/write, NTP fetch, NVS, Serial, settimeofday) reste dans power.cpp.
// =============================================================================

#include <cmath>
#include <ctime>

#include "epoch_util.h"  // EpochUtil::epochAbsDiff (extrait, testé nativement)

namespace ClockDecision {

// Seuils par défaut (parité avec power.cpp / SleepConfig).
...
// Reproduit `timeinfo.tm_year + 1900 < 2024` (rejet) -> renvoie true si OK.
...
// (les bornes/comparaisons sont strictement identiques à l'original).
...
// `_lastDriftSeconds` côté caller. Parité : actual - expected, 0 si millisDiff==0
// ou expected <= 0 (mêmes branches que computeDriftPpm).
