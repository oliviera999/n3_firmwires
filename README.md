...
| **Upload Photos (unifié)** | `uploadphotosserver/` | ESP32-CAM | Un seul code, trois envs : **msp1** (msp1gallery, OTA distant HTTP, deep sleep 600 s, version courante `2.44`), **n3pp** (n3ppgallery, deep sleep 600 s, SD), **ffp3** (ffp3gallery, deep sleep 600 s). Upload et contrôle distant au réveil avec `X-Api-Key` (GET état outputs + POST version firmware, aligné serveur ≥ 5.0.306), retries de connexion, vérification du code HTTP retour. Tables `UploadPhoto*Outputs` (boards 5/6/7). File SD offline-first : le curseur de backlog n'avance qu'après ACK serveur pour éviter la perte silencieuse de photos si l'énumération SD échoue temporairement. Notifications mail SMTP (credentials partagés) : **une fois** au premier démarrage réel (hors réveil deep sleep, flag NVS `upcam/fb_mail`), démarrage/fin OTA, transitions matin/soir du créneau photo. `pio run -e msp1` / `-e n3pp` / `-e ffp3`. |
...
**Flash / esptool « No serial data received » (CH340, env `ffp3`) :** l’environnement **`ffp3`** utilise **`upload_flags = --before=no_reset`** pour ne pas envoyer de séquence DTR/RTS au début (certains adaptateurs ne câblent pas l’auto-bootloader). Procédure : lancer `pio run -e ffp3 -t upload --upload-port COMx`, puis dès que la ligne **Connecting…** apparaît, maintenir **BOOT** (GPIO0), appuyer sur **RST**, relâcher **RST**, relâcher **BOOT** ; ou laisser IO0 à la masse au reset si vous utilisez un FTDI sur UART0.
