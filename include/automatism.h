...
class Automatism {
 ...
 private:
  ...
  // Mise en veille automatique après un certain temps d'activité
  static constexpr unsigned long autoSleepDelayMs = ::SleepConfig::INACTIVITY_DELAY_MS; // 5 minutes
  ...
  struct SleepConfig {
    ...
    uint32_t normalSleepTime = ::SleepConfig::NORMAL_INACTIVITY_DELAY_SEC;   // 10 min normal d'inactivité
    uint32_t errorSleepTime = ::SleepConfig::ERROR_INACTIVITY_DELAY_SEC;    // 5 min si erreurs
