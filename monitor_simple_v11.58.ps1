#!/usr/bin/env pwsh
# Script de monitoring simple - ESP32 v11.58
# Objectif: Validation optimisations mémoire (streaming, seuils sécurité)
...
$logFile = "monitor_simple_v11.58_$timestamp.log"

Write-Host "=== MONITORING SIMPLE 5 MINUTES - ESP32 v11.58 ===" -ForegroundColor Cyan
Write-Host "Objectif: Validation optimisations mémoire (streaming, seuils sécurité)" -ForegroundColor Yellow
...
# Vérifier si le port existe avec PlatformIO
Write-Host "Vérification du port avec PlatformIO..." -ForegroundColor Yellow
$portCheck = & pio device list 2>$null
if ($portCheck -match $Port) {
    Write-Host "✅ Port $Port détecté par PlatformIO" -ForegroundColor Green
} else {
    Write-Host "❌ Port $Port non détecté par PlatformIO" -ForegroundColor Red
...
if (Test-Path $logFile) {
    ...
    # Analyser les optimisations v11.58
    Write-Host "`n🚀 OPTIMISATIONS v11.58:" -ForegroundColor Magenta
    
    # Streaming index.html
    $streamingMatches = [regex]::Matches($logContent, "index\.html streaming")
    if ($streamingMatches.Count -gt 0) {
        Write-Host "✅ Streaming index.html détecté: $($streamingMatches.Count) occurrence(s)" -ForegroundColor Green
    } else {
        Write-Host "⚠️ Streaming index.html non détecté" -ForegroundColor Yellow
    }
    
    # Seuils de sécurité
    $securityMatches = [regex]::Matches($logContent, "Mémoire insuffisante|Heap insuffisant")
    if ($securityMatches.Count -gt 0) {
        Write-Host "⚠️ Refus par manque de mémoire: $($securityMatches.Count) occurrence(s)" -ForegroundColor Yellow
    } else {
        Write-Host "✅ Aucun refus par manque de mémoire" -ForegroundColor Green
    }
    
    # Statistiques streaming
    $streamingStats = [regex]::Matches($logContent, "Streaming stats: (\d+) requests")
    if ($streamingStats.Count -gt 0) {
        $totalRequests = ($streamingStats | ForEach-Object { [int]$_.Groups[1].Value } | Measure-Object -Sum).Sum
        Write-Host "📊 Total requêtes streaming: $totalRequests" -ForegroundColor White
    }
...
Write-Host "Version: v11.58 (Optimisations mémoire)" -ForegroundColor White
