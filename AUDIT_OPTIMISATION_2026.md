...
1. **Découpe des god files ffp5cs** — ✅ **bien avancé** :
   ...
   - `app_tasks.cpp` **1788 → 1093 lignes (−39 %)** — refonte via `app_tasks_internal.h` (interface interne partagée : `g_ctx`, queues, handles en `extern`, définitions inchangées). **6 unités extraites** : `net_request_pool` (pool `NetRequest`), `task_mail` (réserve SMTP + file mails), et les tâches `webTask`/`sensorTask`/`otaTask`/`postSenderTask` (`app_tasks_{web,sensor,ota,post}.cpp`).
   - **Reste** : `netTask` (propriétaire TLS/WebClient — exposerait ~5 handles de tâches) et `automationTask` (orchestrateur) — le cœur temps-réel le plus couplé, à sortir via une conception de contexte dédiée plutôt qu'un déplacement mécanique. *Chaque extraction validée par build `wroom-test` + CI.*
