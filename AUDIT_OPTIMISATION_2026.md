...
1. **Découpe des god files ffp5cs** — ✅ **bien avancé** : `web_server.cpp` **2084 → 1041 lignes (−50 %)** — groupes **WiFi** (`web_routes_wifi.cpp`) et **NVS** (`web_routes_nvs.cpp`) extraits via le pattern `WebRoutes::register*Routes`. **Reste** : découpe d'`app_tasks.cpp` (8 tâches FreeRTOS — **plus risqué**, code de boot/scheduler) et `ota_manager.cpp`. *Chaque extraction validée par build `wroom-test`.*
