  ...
  // Restauration des variables distantes depuis la flash (si disponible)
  {
    ...
    if (_config.loadRemoteVars(cachedJson) && cachedJson.length()>0) {
      ...
      if (!err) {
        ...
        if (doc.containsKey("mailNotif")) emailEnabled = (String(doc["mailNotif"].as<const char*>()) == "checked");
...
void Automatism::applyConfigFromJson(const ArduinoJson::JsonDocument& doc){
  ...
  assignIfPresent("feedEvening", feedEvening);
  // Durées
  if (doc.containsKey("feedBigDur"))   { int v = doc["feedBigDur"].as<int>();   if (v>0) feedBigDur = v; }
  if (doc.containsKey("feedSmallDur")) { int v = doc["feedSmallDur"].as<int>(); if (v>0) feedSmallDur = v; }
  // Seuils
  assignIfPresent("aqThreshold", aqThresholdCm);
  assignIfPresent("tankThreshold", tankThresholdCm);
  if (doc.containsKey("heaterThreshold")) heaterThresholdC = doc["heaterThreshold"].as<float>();
...
bool Automatism::sendFullUpdate(const SensorReadings& readings, const char* extraPairs) {
  ...
  snprintf(data, sizeof(data),
    ...
    "etatPompeAqua=%d&etatPompeTank=%d&etatHeat=%d&etatUV=%d&voltage=%u&" \
    ...
    _acts.isAquaPumpRunning(), _acts.isTankPumpRunning(), _acts.isHeaterOn(), _acts.isLightOn(), readings.voltageMv,
...
void Automatism::handleAutoSleep(const SensorReadings& r) {
  ...
  // Détection d'activité fine
  if (hasSignificantActivity()) {
    // Réinitialise le chrono pour patienter après l'activité
    _lastWakeMs = millis();
    Serial.println(F("[Auto] Auto-sleep différé - activité détectée"));
    return;
  }
  ...
  _power.goToLightSleep(actualSleepDuration);
  ...
  // Restauration des états actionneurs depuis la NVS (préféré)
  {
    ...
    if (loadActuatorSnapshotFromNVS(snapAqua, snapHeater, snapLight)) {
      ...
      if (snapLight)  _acts.light.on();      else _acts.light.off();
      _prevPumpAquaState = snapAqua;
      clearActuatorSnapshotInNVS();
    } else {
      // Fallback RAM (compat)
