...
bool WifiManager::connect(DisplayView* disp) {
  ...
  if (n <= 0) {
    ...
    for (int j = 0; j < n; ++j) {
      Serial.printf("  - %s (%ddBm)%s\n", WiFi.SSID(j).c_str(), WiFi.RSSI(j), WiFi.encryptionType(j)==WIFI_AUTH_OPEN?" [OPEN]":"");
    }
  }
  ...
  for(size_t i=0;i<_count;++i) {
    if(cand[i].present && cand[i].rssi >= ::SleepConfig::WIFI_RSSI_MINIMUM) {
      ...
      Serial.printf("[WiFi] Réseau %s rejeté (RSSI trop faible: %d dBm < %d dBm)\n", 
...
void WifiManager::loop(DisplayView* disp){
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    _retryIntervalMs = _baseRetryMs;
    // Laisser le modem-sleep désactivé
    WiFi.setSleep(false);
    return;
  }
