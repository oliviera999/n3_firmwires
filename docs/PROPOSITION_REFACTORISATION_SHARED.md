...
| **L2 — Primitives** — ✅ *HMAC, heartbeat, timeSync faits* | ~~`computeHmacHex`~~ ✅ ; ~~`n3DataSendHeartbeat`~~ ✅ ; ~~`n3TimeSyncBrokenDown`~~ ✅ ; adoption `n3PrintWakeupReason` **reportée** (la version shared charge la NVS au réveil TIMER — besoin CAM — là où n3pp/msp ne le font qu'en `default` : adoption naïve = risque d'écraser une horloge correcte par un epoch NVS stale ; à trancher) | ffp5cs+n3pp/msp → `n3_hmac`(dédiée)/`n3_data`/`n3_time` | Toutes | **Faible** |
| **L3 — Robustesse capteurs** ✅ *livré* | ~~sensor_failure_manager~~ ✅ (macro log injectable) ; ~~sensor_reading_fallback~~ ✅ (API neutre `N3SensorFallback`) → `n3_analog_sensors` 1.1.0 ; ffp5cs câblé (wrappers). Adoption n3pp/msp = amélioration comportementale, tranche dédiée | ffp5cs → `n3_analog_sensors` | Deep-sleep + ffp5cs | **Faible** |
| **L4 — Offline-first & stats** ✅ *livré (T3)* | ~~`n3_upload`~~ ✅ (multipart streaming, hook `onStats` → voie upload branchée) ; ~~`n3_store_forward`~~ ✅ (peek-commit, drain `n3SfDrain` testé natif) ; ~~ffp5cs `N3NetStats`~~ ✅ (**sous `s_httpMutex`**, v15.17) ; upload câblé v2.66 | upload+ffp5cs → nouvelles libs / `n3_data` | Toutes | **Faible** (ffp5cs : garde mutex) |
| **L5 — Orchestration** — ✅ *T4.1 mail + T4.2 OTA UI faits* | ~~`n3MailNotify`~~ ✅ (`n3_mail` 1.4.0, n3pp 4.57/msp 2.57) ; ~~`n3_ota_ui`~~ ✅ (lib dédiée 1.0.0 + cadence pure `OtaPeriodic` dans `n3_common` 1.6.0, testée natif `test_ota_ui` ; n3pp 4.58/msp 2.58 délèguent le harnais, upload 2.67 la cadence) ; ~~`ota_artifact_select`~~ ✅ (`n3_common` 1.7.0, header pur verbatim, ffp5cs 15.18 consomme ; précondition ArduinoJson devenue triviale : les 5 firmwares sont en 7.4.x) ; ~~rollback OTA 1er boot~~ ✅ **opt-in inactif** (`n3_ota_rollback`, `n3_common` 1.8.0 : décision pure testée natif, runtime sous flag `N3_OTA_ROLLBACK_ENABLE` — activation après validation sur cible, `docs/OTA_ROLLBACK_OPT_IN.md`) | famille → `n3_mail`/`n3_ota` | Deep-sleep surtout | **Faible/Modéré** (parité ArduinoJson v6/v7/toolchains) |
| **L6 — Logging** — ✅ *lib + câblage ffp5cs faits* | ~~`n3_log`~~ ✅ (lib 1.0.0 découplée de `config.h`, échelle = `CORE_DEBUG_LEVEL`, testée natif ; ffp5cs 15.19 délègue `_LOG_IMPL`) ; ⏳ migrer `Serial.print` firmware par firmware (une PR chacun, commencer par upload) | ffp5cs → `n3_log` | Toutes | **Faible sémantique, gros volume** |
| **L7 — Framework** — ✅ *amorcé (T6.0 séquenceur + T6.1 orchestrateur livrés, additifs)* | `n3_app` (cycle deep-sleep à callbacks). ~~T6.0~~ ✅ **additive** : `n3_app_seq.h` (`N3AppContext` + `n3AppNextStep` PUR, `stdint`-only) + `test_app` + CI. ~~T6.1~~ ✅ **additive** : `n3_app.h` — `n3AppRun(cfg, ctx)` (dispatch pur des callbacks `N3AppConfig`, restart/sommeil délégués à `enterSleep`, sans include Arduino, testé en natif). ⏳ suite : **T6.2** adoption firmware par firmware (uploadphotosserver d'abord) | n3pp → `n3_app` | Deep-sleep | **Élevé** — **préalable : harmoniser A6/A7/A8/A10** (✅ faits) |
