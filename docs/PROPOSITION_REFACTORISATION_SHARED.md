...
| **L7 — Framework** — ✅ *amorcé (T6.0 additive : séquenceur pur livré)* | `n3_app` (cycle deep-sleep à callbacks). ~~T6.0~~ ✅ **additive** : `n3_app_seq.h` (`N3AppContext` + `n3AppNextStep` PUR, `stdint`-only, sans consommateur ni include Arduino) + `test_app` + CI. ⏳ suite : wrapper on-target `n3AppRun` (T6.1) puis adoption uploadphotosserver (T6.2) | n3pp → `n3_app` | Deep-sleep | **Élevé** — **préalable : harmoniser A6/A7/A8/A10** (✅ faits) |
