...
void sensorTask(void* pv) {
  ...
  Serial.println(F("[Sensor] Tâche sensorTask démarrée - fréquence 4000ms pour stabilité"));
  
  TickType_t lastWake = xTaskGetTickCount();
  const TickType_t period = pdMS_TO_TICKS(4000);
  for (;;) {
    ...
    esp_task_wdt_reset();
    vTaskDelayUntil(&lastWake, period); // période stable 4 s
  }
}
